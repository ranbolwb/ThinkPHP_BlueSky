...
class IndexController extends Controller {
    ...
    public function menu(){
        ...
        //是否超级管理员
        $dao = M();
        $sql = "select 1 from think_user_role a,think_role b where a.roleid=b.id and b.super=1 and a.userid=".$user["id"];
        $list = $dao->query($sql);
        if($list != null){
            $sql = "select * from think_menu order by code";
        }else {
            $sql = "select c.* from think_user_role a,think_role_menu b,think_menu c where a.roleid=b.roleid and b.menuid=c.id and a.userid=" . $user["id"]." order by c.code";
        }
        $list = $dao->query($sql);
        if($list != null){
            $menu = array();
